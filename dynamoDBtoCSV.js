...
program
  ...
  .option(
    '-k, --keyExpression [keyExpression]',
    'The name of the partition key to filter results on'
  )
  .option(
    '-v, --keyExpressionValues [keyExpressionValues]',
    'The key value expression for keyExpression. See: https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_Query.html'
  )
  .option('-c, --count', 'Only get count, requires -k flag')
  .option(
    '-a, --stats [fieldname]',
    'Gets the count of all occurances by a specific field name (only string fields are supported'
  )
  ...
  .option(
    '-m, --mfa [mfacode]',
    'Add an MFA code to access profiles that require mfa.'
  )
